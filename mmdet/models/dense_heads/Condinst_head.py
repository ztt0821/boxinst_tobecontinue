...
class CondinstHead(AnchorFreeHead):
    """copy from FCOS, because condinst is based on FCOS
    Args:
    ...
    def get_target_points(self, cls_scores):
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores]
        all_level_points = self.get_points(featmap_sizes, bbox_preds[0].dtype,
                                           bbox_preds[0].device)

        ############
        _, _, gt_ids = self.get_targets(all_level_points, gt_bboxes, gt_labels)
        ...
        for i, featmap_size in enumerate(featmap_sizes):
            H, W = featmap_size
            img_ids = torch.arange(num_imgs, device=cls_scores[0].device)
            flatten_img_ids.append(img_ids.repeat_interleave(H * W))
            flatten_level_ids.append(torch.full(
                (num_imgs * H * W, ), i, device=bbox_preds[0].device).long())
        flatten_img_inds = torch.cat(flatten_img_ids)
        flatten_level_inds = torch.cat(flatten_level_ids)
        return flatten_gt_ids, flatten_level_ids, flatten_img_ids
